...
if __name__ == '__main__':
    ...
    theoretical_fg = (np.log(tau**2) -np.log(sigma**2) -1 + (sigma**2 +(mu - nu)**2)/(tau**2))/2
    theoretical_gf = (np.log(sigma**2) -np.log(tau**2) -1 + (tau**2 +(mu - nu)**2)/(sigma**2))/2
    
    print("Theoretical KL(f||g) \t= {0} Theoretical KL(f||g) \t= {1}".format(theoretical_fg, theoretical_gf))
    print("Sample Size | Monte Carlo KL(f||g) | Monte Carlo KL(g||f)")
